...
```bash
python -m server.crawler.cli wikipedia --query "红烧肉" --limit 5
```
...
```python
from server.crawler.browser_crawler import HybridCrawler
...
```bash
# 将JSON文件导入知识库
python -m server.crawler.cli import \
  --file recipes.json \
  --batch-size 20
```
...
- [server/crawler/README.md](../server/crawler/README.md) - 模块文档
