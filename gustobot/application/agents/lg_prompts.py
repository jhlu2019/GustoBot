...
务必选择 `graphrag-query` 以便使用多工具工作流（Graph/LightRAG、预设 Cypher、Text2SQL 等）。
...
## `text2sql-query`
当问题需要访问结构化数据库（如菜谱营养 MySQL 表、菜品用料统计、用户烹饪记录等）时选择此类。
包括但不限于：
- 数据统计查询（如“统计最近一周最受欢迎的菜谱”“计算各菜系的菜谱数量”）
- 数据库表查询（如“列出所有含牛肉的菜谱”“查看最新录入的食材营养信息”）
- 数据分析需求（如“哪道菜的平均评分最高”“常用食材的使用趋势”）
