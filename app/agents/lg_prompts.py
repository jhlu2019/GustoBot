...
- 用户询问菜谱但没有提供具体菜名或食材
- 用户询问营养信息但没有提供具体菜谱或份量
- 用户描述烹饪问题不够具体，无法提供准确帮助
...
## `image-query`
如果用户提供了图片或请求提供图片（如菜谱照片），请将其分类为此类。


## `file-query`
如果用户上传了文件（如菜谱PDF），请将其分类为此类。
"""
...
GET_ADDITIONAL_SYSTEM_PROMPT = """你是一个专业的菜谱助手。你的工作是帮助用户解决各种菜谱相关问题。
